...
def getPostData(item, jsonResult, idx) :
    ...
    jsonResult.append({'idx' : idx, 'title' : title, 'desc' : desc, 'link ' : link, 'address' : address}) 

type = "local"
srcTxt = input("검색어를 입력하세요")
jsonResult = []

jsonResponse = getNaverSearch(type, srcTxt, 1, 5)
...
jsonResponse = getNaverSearch(type,srcTxt,start,5)
...
#csv 파일로 만들기
newsTable = pd.DataFrame(jsonResult, columns=('idx', 'title', 'desc', 'link', 'address'))
